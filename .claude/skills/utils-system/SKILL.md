---
name: utils-system
description: ScreenMiner 的工具类系统开发指南。使用此SKILL当需要创建或修改静态工具类(Util)、扩展方法(Extension)、工具类(Tools)等，包括框架层工具(Assets/FrameWork/Scripts/Utils,Extension,Tools)与游戏层工具(Assets/Scripts/Utils)的分层规范、纯函数原则、命名约定、避免热路径分配、多语言 textId 抽取、Sphere/Box/Ray 搜索工具等。
watched_files:
...
| AnimUtil | 通用 Animator 工具（`GetAnimClipLength` 按片段名关键字读取动画时长）。`partial class`，可与游戏层同名 `AnimUtil` 共享：框架层放与业务无关的通用动画方法，游戏层放业务表现类动画方法 |
| **系统工具** | |
| FileUtil | 文件读写 |
...
游戏层业务工具（依赖 Handler/Manager、读配置表的业务封装等）放在 `Assets/Scripts/Utils/`，按业务领域聚合命名（如 `XxxUtil`）。游戏层**不设独立 Extension 目录**，扩展方法统一放框架层 `Extension/`。具体业务工具清单随游戏层建立在各自业务 skill 中登记。
...
public static class GameUIUtil
{
    ...
    #region 列表项 UI
    public static void SetItemUIForSimple(...) { /* ... */ }
    public static void SetItemUIForDetails(...) { /* ... */ }
    #endregion
}
```
...
{
    public static string GetLanguageText(this MyTypeEnum value) { /* ... */ }
}
...
```csharp
// 循环外缓存频繁访问的上下文对象
var logic = GetSomeLogic();
for (int i = 0; i < hits.Length; i++)
{
    // 用缓存
    var target = logic.GetTargetById(...);
}
...
public static class XxxUtil
{
    public static void Do() { SomeHandler.Instance.xxx; }
}

// ✅ 把依赖作为参数传入
public static class XxxUtil
{
    public static void Do(SomeLogic logic) { logic.xxx; }
}
```

> 注：业务搜索类工具历史上常在内部取单例上下文，是历史原因；新工具应优先采用参数注入。
...
switch (searchType)
{
    case MySearchType.AreaSphere: /* ... */ break;
    // 漏掉新增的其它形状分支
}
...
switch (searchType)
{
    case MySearchType.AreaSphere: /* ... */ break;
    default:
        LogUtil.Log($"[XxxSearchUtil] 未处理的 searchType: {searchType}");
        return null;
}
```
...
按形状分组的搜索/命中工具曾出现「枚举名叫 Box 实际走 Sphere 检测」这类枚举名与实际几何形状不一致的复制粘贴事故。**新增 case 时要按枚举字面语义分组**，可以用枚举值的位运算掩码或独立的几何形状提取方法来避免。
