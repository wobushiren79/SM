---
name: ui-framework
description: ScreenMiner 的 UI 框架开发指南。使用此SKILL当需要创建或修改UI界面、弹窗(Dialog)、气泡(Popup)、提示(Toast)等，包括UI脚本创建、UI管理、UI事件处理等。
watched_files:
...
新增UI后如有需要，在本文档补充对应代码模板与使用示例；改了本 skill `watched_files` 命中的代码须同步更新本文档。
...
| 游戏UI目录（游戏层自建） | `Assets/Scripts/Component/UI/`（建议按 Game/Dialog/Popup/Toast/Common 分子目录） |
...
| 2026-08-21 | PopupShowView 内建出现/消失动画：开关字段 isAnimForShow/isAnimForHide/isAnimWithFade + virtual 方法 AnimForShow/AnimForHide/ShowWithAnim/HideWithAnim（DOScale 弹出/缩回+可选淡出，unscaled，中断恢复）；UIHandler.ShowPopup/HidePopup 收口走 ShowWithAnim/HideWithAnim | - |
