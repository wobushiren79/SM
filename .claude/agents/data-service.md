---
name: data-service
description: 数据服务层开发：BaseDataService<T> 泛型数据服务、JSON/SQLite 数据读写、自动备份。
...
你负责 [FrameWork/Scripts/MVC/](Assets/FrameWork/Scripts/MVC/) 与游戏层 `Assets/Scripts/MVC/` 中的数据服务层开发。
...
└── 游戏层 UserDataService            # 用户存档读写（含自动备份、拆分档）
...
| UserDataService（游戏层） | Assets/Scripts/MVC/Service/UserDataService.cs |
...
### 用户存档拆分约定
主档 `UserData_{slot}` + 同槽目录下独立拆分档（各 `_{slot}` 后缀），`Save` 一并写、`Load` 后注入、`Delete` 一并删；拆分档无备份。详见 data-service-system skill。
